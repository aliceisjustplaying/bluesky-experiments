...
const three = true;
if (three) {
  const feed = await agent.getAuthorFeed({ actor: process.env.BSKY_USERNAME!, limit: 10 });
  const reply = feed.data.feed[0].post; // the latest post
  // console.log(reply);
  const root = (reply.record as any).reply.root; // the root post of the thread
  // console.log(root);
  const replyRoot = await agent.getPostThread({ uri: reply.uri }); // we don't *need* to do this but it's nice to have the full thread
  // console.log(replyRoot);
  // console.log((replyRoot.data.thread.parent as any).post);
  const newBlergh = 'autogenerated link, with a card: https://github.com/aliceisjustplaying/bluesky-experiments';
  const rt = new RichText({ text: newBlergh });
  await rt.detectFacets(agent); // automatically detects mentions and links
  await agent.post({
    $type: 'app.bsky.feed.post',
    text: rt.text,
    ...
    embed: {
      ...
      external: {
        uri: 'https://github.com/aliceisjustplaying/bluesky-experiments',
        title: "alice's bluesky experiments",
        description: "now i'm just playing around with the api some more",
      },
    },
  });
}
